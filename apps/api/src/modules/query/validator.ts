 ...
 * Layer 2: AST parse — must be a pure SELECT statement (node-sql-parser) (Skipped for EDA)
...
export function validateSQL(sql: string, options: ValidationOptions = {}): ValidationResult {
  ...
  // ── Layer 2: AST Parse with node-sql-parser (Skip for EDA mode) ──────────
  if (!options.isEda) {
    ...
    // Basic LIMIT injection for EDA if no parser is used
    if (!/LIMIT\s+\d+/i.test(workingSQL)) {
      workingSQL = workingSQL.replace(/;?\s*$/, '') + ` LIMIT 1000`;
      modifications.push(`Injected LIMIT 1000 for EDA query`);
    }
  }
